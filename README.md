...
> both generated by code from the pipeline outputs (`output/metrics.csv` plus the
> raw subscription rows), so the figures can't drift from the report.
...
source of truth is the **pipeline**, not a hand-typed figure; the *LLM agent*
orchestrates the workflow, narrates the report, and self-checks — it never does
arithmetic. In a fintech context the cost of a wrong figure is high, so financial
calculations are not delegated to a model.

**Figure provenance** (every number traces to code):
- monthly revenue, active/paid/churned users, churn rate, ARPU → `output/metrics.csv`
- validation count & status → `output/validation.json`
- failed-payment leakage, premium-plan share → `data/subscriptions.csv` via `docs/onepagers/build.py`
- retention %, revenue-decline decomposition → derived in code from the metrics
...
Schema (column names & order) · no nulls · unique `(user_id, month)` · exactly
1,000 users · month range · plan↔price mapping · payment-amount consistency ·
lapse-row consistency · active base non-increasing · `paid ≤ active` ·
`churn_rate ∈ [0,1]` · `payment_status` domain · **no reactivation (churned users
don't return)** · **at most one lapse per user** · **revenue reconciliation (CSV
total == metrics total)** · ARPU recomputation · **full monthly metric
recomputation from raw vs `metrics.csv` (proves the whole table is correct, not
just totals)**.

The recomputation re-runs the same `metrics.compute`, so it guards against a stale
or hand-edited table rather than a bug in the metric formula itself; a *second,
independent* revenue formula cross-checks the metric in
`tests/test_revenue_reconciliation_independent.py`. If any check fails, the report
opens with a warning and conclusions are flagged as provisional.
...
result to two audiences — and they are **generated by `build.py` from the pipeline
outputs** (see *Figure provenance* above), so the figures in them cannot drift from
the report (numbers are not hand-typed):
